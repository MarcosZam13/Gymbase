// ProductTable.tsx — Tabla cliente de productos con filtros, badges de stock y acciones

"use client";

import { useState, useDeferredValue } from "react";
import { useRouter } from "next/navigation";
import { Plus, SlidersHorizontal, ChevronRight, ArrowUpDown } from "lucide-react";
...
export function ProductTable({ initialProducts, initialOnlyLowStock = false }: ProductTableProps): React.ReactNode {
  ...
  const filtered = initialProducts.filter((p) => {
    ...
    if (onlyLowStock && p.current_stock > p.min_stock_alert) return false;
    return true;
  });
    ...
    <>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <button
          ...
          style={{
            backgroundColor: onlyLowStock ? "rgba(239,68,68,0.15)" : "#111111",
            border: `1px solid ${onlyLowStock ? "rgba(239,68,68,0.4)" : "#2a2a2a"}`,
            color: onlyLowStock ? "#ef4444" : "#737373",
          }}
        >
        ...
        <button
          type="button"
          onClick={openCreate}
          className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white transition-colors cursor-pointer ml-auto"
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16 rounded-2xl" style={{ backgroundColor: "#0D0D0D", border: "1px solid #1e1e1e" }}>
          <p className="text-[#444] text-sm mb-3">
            {initialProducts.length === 0 ? "No hay productos aún." : "No hay productos que coincidan."}
          </p>
